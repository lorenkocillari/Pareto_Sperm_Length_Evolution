    ...
    for i = 1:n
        for j = i+1:n
            if ~isempty(intersect(vectors{i}, vectors{j}))
                % If shared elements are found, compare lengths and select the shorter one
                ...
                elseif vectorLengths(i) < vectorLengths(j)
                    ...
                    % If both vectors are of equal length and share elements, identify them as not unique
                    isUnique(i) = false;
                    isUnique(j) = false;
                end
            end
