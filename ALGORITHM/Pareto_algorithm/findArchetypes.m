function [PvalueRatio] = findArchetypes(nametable, namefolder, DataPoints, phylogenetic_class, analysis_type,name_class)
%% Description of the function FindArchetypes
auxiliary_spereto;
num_arch    = auxilary.num_arch;
algNum      = auxilary.algNum;
DataDim     = auxilary.dim;
numIter     = auxilary.numIter;
maxRuns     = auxilary.maxRuns;
DataPoints_front = DataPoints;
...
if maxRuns > 0
    ...
    [PvalueRatio] = pvalues_from_tratios(algNum, DataPoints_front, NArchetypes, DataDim, VolArchReal, maxRuns, numIter, phylogenetic_class, ArchsMin,namefolder,name_class);    
...
save(['Results/current_analysis/paretofront_',char(nametable),'_',char(analysis_type),'.mat']);
end