function ContinuousEnrichment(color_class, arch_class, thr_FDR, DataPointsInd,EnMatCont, binSize, contAttrNames, cont_enrich_folder)
%%
    ...
    SEM = cell(1, Numarchs);
    mean_continuous_feature_binned = cell(1, Numarchs);
    for arch = 1:Numarchs 
       tempEnrich =  EnMatCont(DataPointsInd(arch,:),:); 
       Binned = mat2cell(tempEnrich,numPointInBin, numFeatures); 
       mean_continuous_feature_binned{arch} = cell2mat(cellfun(@(x)mean(x),Binned,'UniformOutput',0));
       ...
       pval(arch,:) = ranksum(bin, rest,'tail','both');  % Wilcoxon test between the first bin and the rest of data    
    end
%     isSignificantAfterFDR                                              = mafdr(pval(:),'BHFDR',true)<=ThreshHoldBH; % FDR (NOT IMPLEMENTED IN THIS WAY)
    if sum(pval(:) <= thr_FDR) > 0 % FDR (NOT IMPLEMENTED IN THIS WAY)
        ...
        for ar  = 1:Numarchs
            ...
            shadedErrorBar(bins, mean_continuous_feature_binned{ar},SEM{ar},{'-o','color', color_class{ar},'markerfacecolor',color_class{ar}},1);
            plot(bins, mean_continuous_feature_binned{ar},'Color',color_class{ar},'LineWidth',1);
            if pval(ar) <= 1E-3             
