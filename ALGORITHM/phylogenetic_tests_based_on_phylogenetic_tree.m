...
%% Classify species into groups based on the presence of common ancestors at the specified threshold epoch
    ...
    % Load datapoints which are needed for the analyses
    ...
    tree = phytreeread('Results/current_analysis/sperm_polyresolv_congruOTL_family_230124.tre'); 
...
% % % %     % Find potential duplicates in the dataset
% % % %     [uniqueRows_leafNames, ia_leafNames, ~] = unique(leafNames(idx_in_leafNames), 'stable', 'rows');
% % % %     duplicateIndices_leafNames = setdiff(1:length(leafNames), ia_leafNames);
% % % %     duplicateRows_duplicateIndicesleafNames = leafNames(duplicateIndices_leafNames, :);
% % % %     
% % % %     [uniqueRows_leafNames, ia_leafNames, ~] = unique(all_species_names_underscores(idx_in_all_species_names), 'stable', 'rows');
% % % %     duplicateIndices_leafNames = setdiff(1:length(leafNames), ia_leafNames);
% % % %     duplicateRows_duplicateIndicesleafNames = leafNames(duplicateIndices_leafNames, :);
    ...
    tic   
    % With nIter = 100, choose the following parameters to run the code in ~10 hours
...
% The analyses shown in the paper in Fig.6 have been done using nIter = 30;
% params_Pareto.maxRuns = 300; params_Pareto.numIter = 10;
    ...
    for iter = 1:nIter
        ...
        for sample_size = 1:length(sample_size_ALL)-1
