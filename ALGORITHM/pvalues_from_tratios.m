function [PvalueRatio] = pvalues_from_tratios(algNum, DataPoints_front, NArchetypes, DataDim, VolArchReal, maxRuns, numIter, phylogenetic_class,...
    ArchsMin,namefolder,name_class,thr,analysis_type)
%%  Calculate the p-value from t-ratios
    ...
    if strcmp(analysis_type,'all_species') == 1
	...
	switch algNum
	    case 1 %    algNum=1 :> Sisal (default)
            ...
            plot_randomized_SimplexSisal(DataPoints_front(:,1:NArchetypes),NArchetypes, numIter, phylogenetic_class,thr);
	...
	if algNum < 4 %for first two methods the t-ratio is larger than 1, and we want the minimal one
