...
Table_sperm = readtable([pwd,'/DATA/Pareto_sperm_length_evolution.csv']); % Read the main dataset from the .xlsx file  
...
for class = 1:length(all_classes)
    ...
    plot(x_fit, y_fit, 'Color', color_class{class},'LineWidth',2); 
end
...
mkdir(['Results/Figure S3']); exportgraphics(allfig, ['Results/Figure S3/fecundity_testesmass_bodymass.pdf'])
end